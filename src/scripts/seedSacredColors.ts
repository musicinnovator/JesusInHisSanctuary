    ...
    const colorCombinations = [
      {
        ...
        color_ids: (await supabase.from('sacred_colors').select('id').in('slug', ['blue', 'purple', 'scarlet'])).data?.map(c => c.id) || [],
      ...
      },
      {
        ...
        color_ids: (await supabase.from('sacred_colors').select('id').in('slug', ['gold', 'blue', 'purple', 'scarlet', 'white'])).data?.map(c => c.id) || [],
