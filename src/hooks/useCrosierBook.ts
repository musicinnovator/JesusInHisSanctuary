/**
 * Custom hooks for accessing Crosier's "The Sanctuary" book data
 *
...
import type {
  ...
  BookSearchFilters,
  SearchResults
} from '../types/crosierBook';

/**
 * Hook to fetch book metadata including endorsements
 */
export function useCrosierMetadata() {
  const [metadata, setMetadata] = useState<CrosierBookMetadata | null>(null);
 ...
 */
export function useCrosierChapters(chapterNumber?: number) {
  const [chapters, setChapters] = useState<CrosierChapter[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
 ...
 */
export function useCrosierChapterWithSections(chapterId: string) {
 ...
 */
export function useCrosierScriptures(filters?: {
  book?: string;
  chapterId?: string;
  theme?: string;
}) {
 ...
 */
export function useCrosierConcepts(conceptName?: string) {
 ...
 */
export function useCrosierConceptDetails(conceptId: string) {
 ...
 */
export function useCrosierTypeAntitypes(chapterId?: string) {
 ...
 */
export function useCrosierSearch(filters: BookSearchFilters) {
    ...
    async function performSearch() {
      ...
      try {
        ...
        if (filters.text_query || filters.chapter_number) {
          let chapterQuery = supabase.from('crosier_chapters').select('*');
        ...
        if (filters.scripture_book || filters.theological_theme) {
          let scriptureQuery = supabase.from('crosier_scripture_references').select('*');
        ...
        if (filters.concept_name || filters.text_query) {
          let conceptQuery = supabase.from('crosier_theological_concepts').select('*');
 ...
 */
export function useCrosierBibleBooks() {
